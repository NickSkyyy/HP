...
def orca(graph):
    aa = time.time()
    tmp_fname = '.\\eval\\orca\\tmp_{}.txt'.format(aa)
    f = open(tmp_fname, 'w')
    ...
    output = sp.check_output(['.\\eval\\orca\\orca.exe', 'node', '4', tmp_fname, 'std'])
    output = output.decode('utf8').strip()
    
    idx = output.find(COUNT_START_STR) + len(COUNT_START_STR) + 1
    output = output[idx:]
